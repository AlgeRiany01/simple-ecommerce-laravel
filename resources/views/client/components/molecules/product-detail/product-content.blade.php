...
<div>
    ...
    <p>Category : <a
            href="{{ route('clientCategoryProducts', $dataProductContent->category->name) }}">{!! str_replace('-', ' ', ucwords($dataProductContent->category->name)) !!}</a>
    </p>
    <p><b>Description</b></p>
    ...
    @if ($dataProductContent->stock !== 0)
        <div id="input_div">
            ...
            <input type="button" value="+" id="plus" data-stok="{{ $dataProductContent->stock }}"
                onclick="plus()" class="btn btn-outline-primary">
        </div>
        <div class="d-flex gap-2 mt-4">
            <button class="btn btn-primary btn-small font-secondary add-to-cart"
                data-id-product="{{ $dataProductContent->id }}" data-quantity="1">Add to cart</button>

            @if (isset(session('favorites')[$dataProductContent->id]))
                <form method="POST" action="{{ route('favorites.remove', $dataProductContent->id) }}"
                    style="display: inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger btn-small font-secondary mt-1">
                        <i class="bi bi-heart-fill"></i>
                    </button>
                </form>
            @else
                <form method="POST" action="{{ route('favorites.add', $dataProductContent->id) }}"
                    style="display: inline;">
                    @csrf
                    <button type="submit" class="btn btn-outline-danger btn-small font-secondary mt-1">
